...
/** @brief: WindowPoseDataLayerRegression
 *  Added by KM
 *
**/
template <typename Dtype>
class WindowPoseDataLayerRegression : public BasePrefetchingDataLayer<Dtype> {

 public:
  // Constructor
  explicit WindowPoseDataLayerRegression(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  // Destructor
  virtual ~WindowPoseDataLayerRegression();
  
  // Performs layer-specific setup
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  // Implements forward pass on CPU
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  // Implements forward pass on GPU
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  // Returns layer type
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_WINDOW_POSE_DATA_REGRESSION;
  }

  // Returns the number of blobs taken as input by the layer
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  // Returns the number of blobs produced as output by the layer
  virtual inline int ExactNumTopBlobs() const { return 2; }

 protected:

  // Function to randomize image prefetch
  virtual unsigned int PrefetchRand();
  // Thread that runs by default
  virtual void InternalThreadEntry();

  // RNG object used for randomized prefetch
  shared_ptr<Caffe::RNG> prefetch_rng_;

  // Blobs used to store prefetched data
  Blob<Dtype> prefetch_sin_azimuth_;
  Blob<Dtype> prefetch_cos_azimuth_;
  Blob<Dtype> prefetch_sin_elevation_;
  Blob<Dtype> prefetch_cos_elevation_;
  // Blob to store the set of labels (specified in the variable num_labels_)
  Blob<Dtype> prefetch_labels_;

  // Database to store images
  vector<std::pair<std::string, vector<int> > > image_database_;
  enum WindowField {IMAGE_INDEX, X1, Y1, X2, Y2, SIN_AZIMUTH, COS_AZIMUTH, SIN_ELEVATION, COS_ELEVATION, LABELS, NUM};
  
  // Vectors to hold foreground and background windows (although we use only fg)
  vector<vector<float> > fg_windows_;
  vector<vector<float> > bg_windows_;

  // Blob to store data mean
  Blob<Dtype> data_mean_;

  // Vector to store mean values
  vector<Dtype> mean_values_;

  // Whether or not mean file is present
  bool has_mean_file_;
  // Whether or not mean values are present (if mean file is present, mean values must not be present)
  bool has_mean_values_;
  // Whether to load all images into memory, for faster access
  bool cache_images_;

  // Cache to load images to memory
  vector<std::pair<std::string, Datum > > image_database_cache_;
};
